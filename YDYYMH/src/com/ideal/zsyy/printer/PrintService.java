package com.ideal.zsyy.printer;
...
public class PrintService{
	...
	private byte[] StartBmpToPrintCode(Bitmap bitmap) {
		...
		if (bitmap != null) {
			...
			byte[] byteContent = new byte[(mWidth / 8 + 4)
					* (mHeight + aHeight)];// 锟斤拷印锟斤拷锟斤拷
			byte[] bytehead = new byte[4];// 每锟叫达拷印头
	...
	/**
	 * 追锟斤拷锟侥硷拷锟斤拷使锟斤拷FileOutputStream锟斤拷锟节癸拷锟斤拷FileOutputStream时锟斤拷锟窖第讹拷锟斤拷锟斤拷锟斤拷锟轿猼rue
	 * 
	 * @param fileName
	 * @param content
	 */
