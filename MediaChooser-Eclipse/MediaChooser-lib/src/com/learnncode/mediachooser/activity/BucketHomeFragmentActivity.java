...
//					headerBarCamera.setTag(getResources().getString(R.string.video));
					...
					if(videoFragment == null){
						...
						final VideoFragment newVideoFragment = new VideoFragment();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode ==  Activity.RESULT_OK){
			...
			if (requestCode == MediaChooserConstants.BUCKET_SELECT_IMAGE_CODE){
				...
				//Toast.makeText(BucketHomeFragmentActivity.this,data.getStringExtra("fileUrl"), Toast.LENGTH_SHORT).show();
				Intent intent_img=new Intent();
				intent_img.putExtra("fileType", "img");
				intent_img.putExtra("fileUrl", data.getStringExtra("fileUrl"));
				this.setResult(RESULT_OK, intent_img);
				finish();
...
//						String fileUriString = fileUri.toString().replaceFirst("file:///", "/").trim();
//						android.support.v4.app.FragmentManager fragmentManager = getSupportFragmentManager();
//						VideoFragment bucketVideoFragment = (VideoFragment) fragmentManager.findFragmentByTag("tab2");
